 ...
 *
 * This is the template for the English landing page.
 *
...
<main class="pt-[72px]">
		...
		<div class="relative z-10 max-w-7xl mx-auto px-6 w-full">
			<div class="max-w-[720px]">
				<span class="inline-block bg-white/10 border border-white/20 text-white text-xs font-semibold uppercase tracking-widest px-4 py-2 rounded-full backdrop-blur-sm mb-6">
					The international platform for psychological first aid
				</span>
				...
				<p class="text-white/90 text-lg sm:text-xl leading-relaxed mb-8 max-w-xl">
					Free guide: 3 mistakes that prolong the pain. Enter your email and receive it now.
				</p>

				<form id="lead-form-en" class="flex flex-col sm:flex-row gap-3 max-w-lg">
					<input
						type="email"
						name="email"
						required
				...
				<p id="form-msg-en" class="hidden text-white text-sm mt-3" role="alert"></p>
				<p class="text-white/80 text-sm mt-4">Free | Anonymous | No credit card</p>
			</div>
		</div>
	</section>
		...
		<div class="max-w-7xl mx-auto px-6">
			<div class="grid grid-cols-1 md:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-white/10">
				<div class="px-6 py-6 md:py-0 text-center first:pl-0 last:pr-0">
					<strong class="block text-lg font-semibold mb-1">Psychologist verified</strong>
					<span class="text-sm opacity-75">Evidence-based content</span>
				</div>
				<div class="px-6 py-6 md:py-0 text-center first:pl-0 last:pr-0">
					<strong class="block text-lg font-semibold mb-1">21-day program</strong>
					<span class="text-sm opacity-75">One email per day</span>
				</div>
				<div class="px-6 py-6 md:py-0 text-center first:pl-0 last:pr-0">
					<strong class="block text-lg font-semibold mb-1">Anonymous</strong>
					<span class="text-sm opacity-75">Nobody knows. Only you.</span>
				</div>
			</div>
		</div>
	</section>
				...
				<article class="relative bg-card-active border border-card-active-border rounded-lg overflow-hidden flex flex-col justify-between">
					...
					<div class="p-10 sm:p-12 flex-1 flex flex-col justify-between">
						<div>
							...
							<h3 class="text-2xl font-semibold mb-2">After a breakup</h3>
							<p class="text-gray-600 mb-8">When thoughts of them won't go away.</p>
						</div>
						
						<?php
						$stripe_en = get_theme_mod( 'stripe_link_en', 'https://buy.stripe.com/REPLACE_ENGLISH_LINK' );
						$is_en_ready = ( ! empty( $stripe_en ) && strpos( $stripe_en, 'REPLACE' ) === false );
						if ( $is_en_ready ) :
						?>
							<a href="<?php echo esc_url( $stripe_en ); ?>" class="block w-full text-center bg-primary text-white font-semibold py-4 rounded hover:bg-accent transition-colors">
								Start program ->
							</a>
						<?php else : ?>
							<button disabled class="block w-full text-center bg-gray-300 text-gray-500 font-semibold py-4 rounded cursor-not-allowed">
								Start program ->
							</button>
						<?php endif; ?>
					</div>
				...
				<article class="relative bg-[#fafafa] opacity-70 rounded-lg overflow-hidden cursor-default flex flex-col justify-between">
					...
					<div class="p-10 sm:p-12 flex-1 flex flex-col justify-between">
						<div>
							...
							<h3 class="text-2xl font-semibold mb-2">Insomnia</h3>
							<p class="text-gray-600 mb-8">Your path to restful sleep.</p>
						</div>
		...
		<div class="max-w-7xl mx-auto px-6">
			...
			<div class="flex flex-col md:flex-row md:items-center gap-8 md:gap-0">
				...
				<div class="flex items-start gap-5 flex-1">
					...
					<div class="flex-1">
						<h3 class="text-lg font-semibold mb-1">Enter your email</h3>
						<p class="text-gray-600 text-sm leading-relaxed">Get the free guide instantly.</p>
					</div>
				...
				<div class="flex items-start gap-5 flex-1">
					...
					<div class="flex-1">
						<h3 class="text-lg font-semibold mb-1">Read it</h3>
						<p class="text-gray-600 text-sm leading-relaxed">5 minutes. 3 mistakes you're probably making.</p>
					</div>
					<div class="hidden md:block w-px h-12 bg-card-active-border mx-6 shrink-0 self-center" aria-hidden="true"></div>
				</div>
				<!-- Step 3 -->
				<div class="flex items-start gap-5 flex-1">
					...
					<div class="flex-1">
						<h3 class="text-lg font-semibold mb-1">Start the 21-day program</h3>
						<p class="text-gray-600 text-sm leading-relaxed">EUR 39.90. One email per day. Step by step.</p>
					</div>
				</div>
			</div>
		</div>
	...
	<section class="bg-primary text-white py-24 text-center">
		<div class="max-w-3xl mx-auto px-6">
			<h2 class="text-3xl sm:text-4xl font-semibold mb-4">21-day breakup recovery program</h2>
			<p class="text-xl sm:text-2xl mb-8 flex items-center justify-center flex-wrap gap-3">
				...
				<span class="opacity-90 text-lg">| Instant access</span>
			</p>
			<?php if ( $is_en_ready ) : ?>
				<a href="<?php echo esc_url( $stripe_en ); ?>" class="inline-block bg-white text-primary font-semibold text-lg px-8 py-4 rounded-lg hover:bg-light-bg transition-all hover:-translate-y-0.5 shadow-lg">
					Start now ->
				</a>
			<?php else : ?>
			...
			<?php endif; ?>
			<p class="mt-5 text-sm opacity-80">Anonymous | Instant access | No risk</p>
		</div>
	</section>
</main>
...
<footer class="bg-light-bg border-t border-footer-border py-12">
	<div class="max-w-7xl mx-auto px-6">
		...
		<div class="text-sm mb-4 flex items-center gap-2">
			<a href="<?php echo esc_url( site_url('/privacy-policy') ); ?>" class="text-primary font-medium hover:underline">Privacy Policy</a>
			<span class="text-gray-400">|</span>
			<a href="<?php echo esc_url( site_url('/terms-of-service') ); ?>" class="text-primary font-medium hover:underline">Terms of Service</a>
		</div>
...
<div id="wp-cookie-banner" class="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 shadow-2xl z-[9999] transition-transform duration-300 translate-y-full">
	<div class="max-w-7xl mx-auto px-6 py-4 flex flex-col sm:flex-row items-center justify-between gap-4">
		...
		<div class="flex items-center gap-3 shrink-0">
			<button id="cookie-decline" class="px-4 py-2 text-sm font-medium text-gray-500 hover:text-gray-800 transition-colors">Decline</button>
			<button id="cookie-accept" class="bg-primary text-white px-5 py-2 rounded text-sm font-semibold hover:bg-accent transition-colors">Accept</button>
		</div>
	</div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
	...
	var form = document.getElementById('lead-form-en');
	var formMsg = document.getElementById('form-msg-en');
	if (form) {
		form.addEventListener('submit', function(e) {
			...
			fetch('<?php echo esc_url( site_url("/api/leads") ); ?>', {
				...
				if (res.ok) {
					btn.innerText = 'Sent!';
					formMsg.innerText = 'Sent! Check your email for your free guide.';
					...
					formMsg.innerText = 'Something went wrong. Please check your email and try again.';
	...
	acceptBtn.addEventListener('click', function() {
		...
		// Trigger custom GA or Meta Pixel initialization if defined
	});
